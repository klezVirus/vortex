...
def getUsers(engine, args):
    ...
    c = CrossLinked(engine, args.company_name, args.timeout, 3, args.header, args.proxy, args.jitter, args.safe,
                    args.debug)
...
def crosslinked_run(**kwargs):
    global logger
    logger = setup_consoleLogger(logging.DEBUG if kwargs["debug"] else logging.INFO)
...
if __name__ == '__main__':
    banner()
    args = argparse.ArgumentParser(description="", formatter_class=argparse.RawTextHelpFormatter,
    ...
    s.add_argument('--search', dest='engine', type=str, default='google,bing',
                   help='Search Engine (Default=\'google,bing\')')
    s.add_argument("--safe", dest="safe", action='store_true',
                   help="Only parse names with company in title (Reduces false positives)")

    o = args.add_argument_group("Output arguments")
    o.add_argument('-f', dest='nformat', type=str, required=True,
                   help="Format names, ex: 'domain\\{f}{last}', '{first}.{last}@domain.com'")
    o.add_argument('-o', dest='outfile', type=str, default='names.txt',
    ...
    pr.add_argument('--proxy-file', dest='proxy', default=False, type=lambda x: file_exists(args, x),

...
class verify_nameservers(multiprocessing.Process):
    ...
    def __init__(self, target, record_type, resolver_q, resolver_list, wildcards):
        ...
        # This needs testing!
        self.record_type = dns.rdatatype.A
        if record_type == "AAAA":
            self.record_type = dns.rdatatype.AAAA
    ...
    def find_wildcards(self, host):
        ...
        try:
            ...
            wildtest = self.resolver.resolve(uuid.uuid4().hex + ".com", dns.rdatatype.A)
            if len(wildtest):
        ...
        while True:
            try:
                ...
                if not record_type or record_type == "A":
                    resp = self.resolver.resolve(host, dns.rdatatype.A)
                    # Crawl the response
                ...
                if record_type == "CNAME":
                    ...
                    for x in range(20):
                        try:
                            resp = self.resolver.resolve(host, dns.rdatatype.CNAME)
                    ...
                    rt = dns.rdatatype.from_text(record_type)
                    # All other records:
                    return self.resolver.resolve(host, rt)
...
if __name__ == "__main__":
    ...
    parser.add_option("-r", "--resolvers", dest="resolvers", default=os.path.join(base_path, "resolvers.txt"),
                      type="string",
                      help="(optional) A list of DNS resolvers, if this list is empty it will OS's internal resolver "
    ...
    parser.add_option("-f", "--filter_subs", dest="filter", default="",
                      type="string",
                      help="(optional) A file containing unorganized domain names which will be filtered into a list "
                           "of subdomains sorted by frequency.  This was used to build names.txt.")
    ...
    if options.targets != "":
        ...
        # ./subbrute.py google.com gmail.com yahoo.com

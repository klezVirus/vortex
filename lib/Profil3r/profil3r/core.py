...
try:
    from lib.Profil3r.profil3r.modules.email import email
    from lib.Profil3r.profil3r.modules.social import facebook, twitter, tiktok, instagram
    from lib.Profil3r.profil3r.modules.porn import onlyfans
    from lib.Profil3r.profil3r.colors import Colors
    ...
    def print_results(self, element):
        if element in result:
            ...
            if not element_results["accounts"]:
                print("\n" + Colors.BOLD + "└──" + Colors.ENDC + Colors.OKGREEN + " {} ✔️".format(
                    element.upper()) + Colors.ENDC + Colors.FAIL + " (No results)" + Colors.ENDC)
                return
                # Results
            else:
                print("\n" + Colors.BOLD + "└──" + Colors.ENDC + Colors.OKGREEN + " {} ✔️".format(
                    element.upper()) + Colors.ENDC)
            ...
            # Emails case
                ...
                for account in element_results["accounts"]:
                    ...
                    if account["breached"]:
                        print(
                            Colors.BOLD + "   ├──" + Colors.ENDC + Colors.HEADER + email + Colors.FAIL + "[BREACHED]" + Colors.ENDC)
                    # Safe account
                    else:
                        print(
                            Colors.BOLD + "   ├──" + Colors.ENDC + Colors.HEADER + email + Colors.OKGREEN + "[SAFE]" + Colors.ENDC)
